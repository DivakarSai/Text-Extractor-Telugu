...
class TesseractOCR(OCR):
  ...
  def _runocr(self, images, language = None):
    ...
    with open(output_path + '.tsv', 'r', newline='') as tsv_file:
      reader = csv.reader(tsv_file, delimiter='\t')
      ocr_output = list(reader)
    
    #Format output to more usable form.
    result = []
    for row in ocr_output:
      # Assuming 'level' is at index 0 and 'page_num' is at index 1 in each row
      if row[0] == '1':  # Accessing 'level' by index
          last_page = row[1]  # Accessing 'page_num' by index
          result.append([])

      #Check if row is a word
      if row[0] == '5' and len(row[1]) > 0 and row[1] != ' ':
        word = {}
        word['text'] = row[1]
        word = {}
        word['text'] = row['text']
